declare global {
    ...
    interface BoilerplateEngine {
        ...
        drawLine(strokeStyle: StrokeStyle, x1: number, y1: number, x2: number, y2: number);
        drawLine(strokeStyle: StrokeStyle, p1: Point2, p2: Point2);
        ...
        pointInRectangle(p: Point2, rect: Rect);
        randomDouble(min: number, max: number): number;
        randomInt(min: number, max: number): number;
        randomPoint(): Point2;
        pointInPolygon(p: Point2, poly: Polygon);
        ...
        mapMany<T, U>(arr: T[], f: (T) => U[]): U[];
        enumerate(n: number): number[];
        enumerate<T>(arr: T[]): (T | number)[];
        any<T>(arr: T[], pred: (T) => boolean): boolean;
        zip<T, U, V>(a: T[], b: U[], f: (T, U) => V): V[];
        zip3<T, U, V, W>(a: T[], b: U[], c: V[], f: (t: T, u: U, v: V) => W): W[];
        arrayify<T>(val: T): T | T[];

        createLinearGradient(p1: Point2, p2: Point2, c1: Color, c2: Color);
        setCompositeOperation(val: string);
    }
